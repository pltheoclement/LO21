...
void ExpressionManager::removeExpression(Expression& e){
	...
	if (i==nb) throw CompUTException("elimination d'une Expression inexistante");
	delete exps[i];
	i++;
...
Expression& Item::getExpression() const { 
		if (exp==nullptr) throw CompUTException("Item : tentative d'acces a une expression inexistante"); 
		return *exp; 
}


void Stack::agrandissementCapacite() {
...
void Stack::affiche(QTextStream& f) const{
    f<<"********************************************* \n";
    f<<"M : "<<message<<"\n";
    f<<"---------------------------------------------\n";
...
Expression& Stack::top() const { 
	
	if (nb==0) throw CompUTException("aucune expression sur la stack");
